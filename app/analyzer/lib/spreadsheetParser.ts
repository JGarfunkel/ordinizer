...
export interface ISpreadsheetParser {
  ...
  parseNames(text: string): { name: string, type: string };
...
export class DefaultSpreadsheetParser implements ISpreadsheetParser {
  ...
  parseName(text: string): string | null  {
    ...
    const { name: entityName, type: entityType } = this.parseNames(text);
    return entityName && entityType ? `${entityName}-${entityType}` : entityName;
  }

  parseNames(text: string): { name: string, type: string } {
    if (!text?.trim()) return { name: "", type: "" };
    ...
    if (!cleanType) return { name: cleanName, type: "" };
    return { name: cleanName, type: cleanType.replace(/[^a-zA-Z0-9]/g, "") };
  }
  ...
  getEntityPrefix(): string {
    const state = this.realm?.territory || loadSpreadsheetExtractionProperties().state;
    return `${state}-`;
  }

  getStateCode(): string {
    if (this.realm?.territory) return this.realm.territory;
